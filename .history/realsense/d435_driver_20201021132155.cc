...
/* 函数作用为了把得到的图转换成二值图 */
std::vector<cv::Mat> D435::thresholding2gray(
    std::vector<cv::Mat> data, cv::Mat mean_depth,
    const std::vector<double> &thread_data, int h, int nums, cv::Rect ROI,cv::Mat result_image) {
  for (int i = 0; i < data.size(); i++) {    // 处理第i副图像
      if()
  }
}
