...
void norm_image(cv::Mat& src) {
  ...
  for (int i = 0; i < nr; i++) {
    for (int j = 0; j < nc; j++) {
      src.at<uchar>(i, j) =
          ((src.at<uchar>(i, j) - min_val) / (max_val - min_val)) * 255;
    }
  }
  // return src;
}
...
vector<vector<Point> > find_obstacle(Mat& dep, int thresh = 20,
...
int main(int argc, char** argv) try {
  ...
  while (1) {
    ...
    Mat color(Size(width, height), CV_8UC3, (void*)color_frame.get_data(),
              Mat::AUTO_STEP);
    Mat depth_raw(Size(width, height), CV_16UC1, (void*)depth_frame.get_data(),
                  Mat::AUTO_STEP);
    cv::Mat depth_copy;
    cv::Mat depth;
    ...
    if (ve_rect.size() > 0) {
      for (int i = 0; i < ve_rect.size(); i++) {
        cout << "object  " << i << ":" << ve_rect[i].x << "," << ve_rect[i].y
             << endl;
