...
class PlaneModel : public GRANSAC::AbstractModel<3> {
 protected:
  ...
  GRANSAC::VPFloat m_DistDenominator;   // = sqrt(a^2 + b^2+c^2). Stored for
                                        // efficiency reasons
  ...
  virtual GRANSAC::VPFloat ComputeDistanceMeasure(
    ...
    GRANSAC::VPFloat Numer =
        fabs(m_a * ExtPoint3D->m_Point3D[0] + m_b * ExtPoint3D->m_Point3D[1] +
             m_c * ExtPoint3D->m_Point3D[2] + m_d);
    GRANSAC::VPFloat Dist = Numer / m_DistDenominator;

    //// Debug
    // std::cout << "Point: " << ExtPoint2D->m_Point2D[0] << ", " <<
    // ExtPoint2D->m_Point2D[1] << std::endl; std::cout << "Line: " << m_a << " x
    // + " << m_b << " y + "  << m_c << std::endl; std::cout << "Distance: " <<
    // Dist << std::endl << std::endl;

    return Dist;
  };

 public:
  PlaneModel(const std::vector<std::shared_ptr<GRANSAC::AbstractParameter>>&
                 InputParams) {
  ...
  virtual void Initialize(
    ...
    // (Point2->m_Point2D[0] - Point1->m_Point2D[0]); // Slope m_d =
    // Point1->m_Point2D[1] - m_m * Point1->m_Point2D[0]; // Intercept
    ...
    m_DistDenominator = sqrt(m_a * m_a + m_b * m_b +
                             m_c * m_c);  // Cache square root for efficiency
    m_PlaneCoefs[0] = m_a;
    m_PlaneCoefs[1] = m_b;
    m_PlaneCoefs[2] = m_c;
    m_PlaneCoefs[3] = m_d;
