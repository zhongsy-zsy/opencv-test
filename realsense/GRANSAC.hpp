...
class RANSAC {
 private:
  ...
  std::vector<std::mt19937>
      m_RandEngines;  // Mersenne twister high quality RNG that support *OpenMP*
                      // multi-threading
...
#pragma omp parallel for num_threads(5)
    for (int i = 0; i < m_MaxIterations; ++i) {
      ...
      std::shuffle(
          RemainderSamples.begin(), RemainderSamples.end(),
      ...
      // RemainderSamples.begin() + t_NumParams); // Remove the model data points
      // from consideration. 2018: Turns out this is not a good idea
