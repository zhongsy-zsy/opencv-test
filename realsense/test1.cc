...
void norm_image(cv::Mat& src) {
  ...
  for (int i = 0; i < nr; i++) {
    for (int j = 0; j < nc; j++) {
      src.at<ushort>(i, j) =
          ((src.at<ushort>(i, j) - min_val) / (max_val - min_val)) * 255;
    }
  }
  // return src;
}
...
double caculate(cv::Mat depth, cv::Rect rect, vector<double>& res) {
  ...
  // std::cout<<"最小距离"<<mindiatance<<std::endl;
  // //因为感兴趣区域内会存在零点所以需要弄掉
...
Point2f GetPointAfterRotate(Point2f inputpoint, Point2f center, double angle) {
  Point2d preturn;
  preturn.x = (inputpoint.x - center.x) * cos(-angle) -
              (inputpoint.y - center.y) * sin(-angle) + center.x;
  preturn.y = (inputpoint.x - center.x) * sin(-angle) +
              (inputpoint.y - center.y) * cos(-angle) + center.y;
  return preturn;
}
Point GetPointAfterRotate(Point inputpoint, Point center, double angle) {
  Point preturn;
  preturn.x = (inputpoint.x - center.x) * cos(-1 * angle) -
              (inputpoint.y - center.y) * sin(-1 * angle) + center.x;
  preturn.y = (inputpoint.x - center.x) * sin(-1 * angle) +
              (inputpoint.y - center.y) * cos(-1 * angle) + center.y;
  return preturn;
}
double getOrientation(vector<Point>& pts, Point2f& pos, Mat& img) {
  ...
  //计算出直线，在主要方向上绘制直线
  // line(img, pos, pos + 0.02 * Point2f(eigen_vecs[0].x * eigen_val[0],
  // eigen_vecs[0].y * eigen_val[0]) , CV_RGB(255, 255, 0)); line(img, pos, pos +
  // 0.02 * Point2f(eigen_vecs[1].x * eigen_val[1], eigen_vecs[1].y *
  // eigen_val[1]) , CV_RGB(0, 255, 255));
...
vector<vector<Point> > find_obstacle(Mat& depth, int thresh = 20,
  ...
  Mat element =
      getStructuringElement(MORPH_RECT, Size(3, 3));  //核的大小可适当调整
  // Mat out;
  //进行开操作
  ...
  for (int i = 0; i < contours.size(); i++) {
    ...
    //             contours[i][j] =
    //             GetPointAfterRotate(contours[i][j],(Point)*pos,dOrient);
    //         Rect rect = boundingRect(contours[i]);//轮廓最小外接矩形
    //         RotatedRect rotateRect =
    //         RotatedRect((Point2f)rect.tl(),Point2f(rect.br().x,rect.tl().y),(Point2f)rect.br());
    //         //将角度转换回去并绘图
    //         Point2f rect_points[4];
    //         rotateRect.points( rect_points ); //求出矩形的四个顶点
    //         for (size_t j = 0;j<4;j++)
    //             rect_points[j] =
    //             GetPointAfterRotate((Point)rect_points[j],(Point)*pos,-dOrient);
    //         for( size_t j = 0; j < 4; j++ )
    //             line( drawing, rect_points[j],
    //             rect_points[(j+1)%4],Scalar(255,255,0),2);
    //         //得出结果
...
int main(int argc, char* argv[]) {
  ...
  rs2::pipeline pipe;  //创建一个通信管道//https://baike.so.com/doc/1559953-1649001.html
                       //pipeline的解释
  ...
  while (true) {
    ...
    Mat color(Size(Width, Height), CV_8UC3, (void*)color_frame.get_data(),
              Mat::AUTO_STEP);
    Mat depth(Size(Width, Height), CV_16U, (void*)depth_frame.get_data(),
              Mat::AUTO_STEP);
